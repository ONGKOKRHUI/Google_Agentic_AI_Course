...
def generate_images(
    ...
    Args:
        num_images: Number of containers to ship
    ...
    # -----------------------------------------------------------------------------------------------
    # -----------------------------------------------------------------------------------------------
    # SCENARIO 1: Small number (≤1 images) auto-approve
    if num_images <= LARGE_GENERATION_THRESHOLD:
        return {
            "status": "approved",
            "order_id": f"ORD-{num_images}-AUTO",
            "num_images": num_images,
            "message": f"Generation auto-approved: Generate {num_images} images of {description}",
        }

    # -----------------------------------------------------------------------------------------------
    # -----------------------------------------------------------------------------------------------
    # SCENARIO 2: This is the first time this tool is called. Large # generations need human approval - PAUSE here.
    if not tool_context.tool_confirmation:
        tool_context.request_confirmation(
            hint=f"⚠️ Large generation: {num_images} images of {description}. Do you want to approve?",
    ...
    if tool_context.tool_confirmation.confirmed:
        return {
            ...
            "message": f"Generation approved: Generate {num_images} images of {description}",
        }
    else:
        return {
            "status": "rejected",
            "message": f"Generation rejected: Generate {num_images} images of {description}",
        }
...
#individual LlmAgent are stateless and no memory maintained
shipping_app = App(
...
async def run_shipping_workflow(query: str, auto_approve: bool = True):
    ...
    # -----------------------------------------------------------------------------------------------
    # -----------------------------------------------------------------------------------------------
    # STEP 1: Send initial request to the Agent. If num_images > 5, the Agent returns the special `adk_request_confirmation` event
